#Helper for cis/trans annotation

#The local copy of genes created here:
#library(EnsDb.Hsapiens.v75)  # GRCh37-compatible
...
#gene_ref <- data.frame(
 ...
 # hgnc_symbol = genes$gene_name,
 # chromosome_name = as.character(seqnames(genes)),
#  start_position = start(genes),
#  end_position = end(genes)
#)
#setwd("/dir/proteomewide/data/gene_ref")
#saveRDS(gene_ref, "gene_ref_grch37.rds")
...
#Function to annotate cis/trans variants locally (ie without calling BiomaRt)

annotate_cis_trans_local <- function(
  dat,
  gene_ref,
  cis_region = 500000,
  chr_col = "chr.exposure",
  pos_col = "pos.exposure",
  values_col = "gene.exposure",
  filter = "ensembl_gene_id",
  missing = "include"
){

  if (!(missing %in% c("include", "drop"))) {
    warning("missing must be 'include' or 'drop'")
    return(dat)
  }

  if (!(filter %in% c("ensembl_gene_id", "hgnc_symbol"))) {
    warning("filter must be 'ensembl_gene_id' or 'hgnc_symbol'")
    return(dat)
  }

  # Ensure chromosome format is consistent 
  dat[[chr_col]] <- as.character(dat[[chr_col]])
  gene_ref$chromosome_name <- as.character(gene_ref$chromosome_name)

  # Merge gene coordinates
  merged <- merge(
    dat,
    gene_ref,
    by.x = values_col,
  ...
  # Identify rows with complete annotation
  valid <- !is.na(merged$chromosome_name) &
           !is.na(merged$start_position) &
           !is.na(merged$end_position)

  # Annotation
  merged$cistrans[valid] <- ifelse(
    merged[[chr_col]][valid] == merged$chromosome_name[valid] &
    as.numeric(merged[[pos_col]][valid]) >= (merged$start_position[valid] - cis_region) &
    as.numeric(merged[[pos_col]][valid]) <= (merged$end_position[valid] + cis_region),
    "C",
    "T"
  )
  ...
  if (missing == "drop") {
    merged <- merged[merged$cistrans != "U", ]
  }
...
#Call UKB GWAS files
ukb<- list.files(path = "/dir/data/ukb/clean_cd/gwas/", recursive = TRUE)
...
setwd("/dir/data/ukb/clean_cd/gwas/")
...
fwrite(log,"log_ukb.txt", sep="\t", quote=F, row.names=F, col.names=F, append=TRUE)
} else { 
fwrite(log, "log_ukb.txt", sep="\t", quote=F, row.names=F, append=FALSE)
}
...
#Estimate F and annotate cis/trans
  ...
  variants_tmp <- mrpipeline::calc_f_stat(variants_clumped)
  
  gene_ref <- readRDS("/dir/proteomewide/data/gene_ref/gene_ref_grch37.rds")

variants_final <- annotate_cis_trans_local(
  variants_tmp,
  gene_ref,
  cis_region = 500000,
  chr_col = "chr.exposure",
...
setwd("/dir/proteomewide/data/instruments")
if(file.exists("instruments_ukb.txt") == TRUE) {
fwrite(variants_final_save,"instruments_ukb.txt", sep="\t", quote=F, row.names=F, col.names=F, append=TRUE)
} else { 
fwrite(variants_final_save, "instruments_ukb.txt", sep="\t", quote=F, row.names=F, append=FALSE)
}
...
setwd("/dir/proteomewide/data/regions")
if(file.exists("topregions_ukb.txt") == TRUE) {
  fwrite(region,"topregions_ukb.txt", sep="\t", quote=F, row.names=F, col.names=F, append=TRUE)
} else { 
  fwrite(region, "topregions_ukb.txt", sep="\t", quote=F, row.names=F, append=FALSE)
}
...
#Saving the output
setwd("/dir/proteomewide/data/regions/exposure")
...
fwrite(variants_region, file_out_snp[o], sep="\t", quote=F, row.names=F, col.names=T)}
}})
...
file_out_snp<- paste0(snp, ".txt")
fwrite(variants_region, file_out_snp, sep="\t", quote=F, row.names=F, col.names=T)}
}
}
...
region_files<- list.files(path="/dir/proteomewide/data/regions/exposure", 
pattern="*.txt$", full.names=TRUE, recursive=FALSE)

psychiatric_files<- list.files(path="/dir/proteomewide/psychiatric_data/clean", 
pattern="*.txt$", full.names=TRUE, recursive=FALSE)

psychiatric_files<- psychiatric_files[c(4,8,9,10,11,12)]
...
setwd("/dir/proteomewide/data/regions/outcome")
fwrite(psychiatric_region, psychiatric_region_file_name, sep="\t", quote=F, row.names=F, col.names=T)
...
mol_region_files<- list.files(path="/dir/proteomewide/data/regions/exposure", 
pattern="*.txt$", full.names=TRUE, recursive=FALSE)

psych_region_files<- list.files(path="/dir/proteomewide/data/regions/outcome", 
pattern="*.txt$", full.names=TRUE, recursive=FALSE)
...
mol_trait <- list.files(path = "/dir/proteomewide/data/regions/exposure", pattern=pattern, recursive = TRUE)
print(mol_trait)
pheno<- list.files(path = "/dir/proteomewide/data/regions/outcome", pattern=pattern, recursive = TRUE)
print(pheno)

#Assign to the identified files the full directory so that pwcoco can work 
mol_trait_dir<- paste0("/dir/proteomewide/data/regions/exposure/", mol_trait)
print(mol_trait_dir)
pheno_dir<- paste0("/dir/proteomewide/data/regions/outcome/", pheno)
print(pheno_dir)
...
out_cmd<- paste("--out", paste0("/dir/proteomewide/results/colocalisation/v1_"))
print(out_cmd)
